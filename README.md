<div align="center">
  <br />
  <img width="300" height="150" alt="Group 68" src="https://github.com/user-attachments/assets/210c606a-66c9-4743-9c67-30c700b84ab0" />

  <br />
</div>

## 🧑🏻‍💻 프로젝트 소개

> 14년째 함께하고 있는 청군이의 하루하루를 기록하고,
> 가족 모두가 언제든 꺼내볼 수 있는 우리 가족 반려동물 SNS 🐾
>
> "오늘 청군이 뭐 했어?" 카톡 대신, 여기서 확인하세요!
> [👉🏻 청군이네 바로가기](https://cheonggun-house.vercel.app/)

청군이네는 오로지 청군이만을 위한 공간으로 인증된 사용자만 게시글 작성이 가능합니다.
혹시 청군이 사진을 올리고 싶은 경우에 회원가입 후 게시판을 통해 인증 요청을 보내주세요!

## 💻 주요 기능

### [유저 인증]

> 3단계 회원가입 (계정 정보 → 프로필 설정 → 반려동물 등록)
> Zod 유효성 검사 (이메일 형식, 비밀번호 복잡도)
> Supabase Auth 기반 로그인/로그아웃

### [게시글]

> 다중 이미지 업로드 및 캐러셀 뷰
> 태그 추가 (산책, 간식, 목욕, 병원, 훈련, 일상, 여행, 장난감, 생일, 미용)
> 촬영 날짜 지정
> React Query 무한스크롤 피드
> 게시글 수정/삭제 (본인 게시물)

### [좋아요 · 댓글 · 북마크]

> 게시글 좋아요 / 댓글 좋아요
> 댓글 작성/삭제
> 게시글 북마크 저장 및 프로필에서 조회

### [알림]

> 좋아요, 댓글, 댓글 좋아요 시 알림 생성
> 미읽 알림 카운트 및 일괄 읽음 처리
> 알림 클릭 시 해당 게시글로 이동

### [검색]

> 태그 기반 게시글 검색 (썸네일 그리드)
> 닉네임 기반 사용자 검색

### [사용자 프로필]

> 프로필 조회 (반려동물 정보 포함)
> 게시물 날짜별 그룹핑 표시
> 북마크 탭 (본인 프로필)
> 닉네임, 자기소개, 비밀번호, 반려동물 정보 수정

### [관리자]
> 사용자 권한 관리 (approved ↔ viewer 토글)
> approved: 게시글 작성 가능 / viewer: 조회만 가능


----


<div>
  <img src="https://img.shields.io/badge/next.js-v14.2.5-000000?style=for-the-badge&logo=nextdotjs&logoColor=white">
  <img src="https://img.shields.io/badge/typescript-v5.0-3178C6?style=for-the-badge&logo=typescript&logoColor=white">
  <img src="https://img.shields.io/badge/reactquery-v5.52.2-FF4154?style=for-the-badge&logo=reactquery&logoColor=white">
  <img src="https://img.shields.io/badge/reacthookform-EC5990?style=for-the-badge&logo=reacthookform&logoColor=white">
  <img src="https://img.shields.io/badge/zod-3E67B1?style=for-the-badge&logo=zod&logoColor=white">
 
</div>

<div>
   <img src="https://img.shields.io/badge/zustand-%2320232a.svg?style=for-the-badge&logo=zustand&logoColor=%2361DAFB"/>
  <img src="https://img.shields.io/badge/tailwindcss-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white">
  <img src="https://img.shields.io/badge/shadcnui-000000?style=for-the-badge&logo=shadcnui&logoColor=white">
  <img src="https://img.shields.io/badge/supabase-3FCF8E?style=for-the-badge&logo=supabase&logoColor=white">
   <img src="https://img.shields.io/badge/vercel-000000?style=for-the-badge&logo=vercel&logoColor=white">
   <img src="https://img.shields.io/badge/npm-CB3837?style=for-the-badge&logo=npm&logoColor=white">
  <img src="https://img.shields.io/badge/claude-D97757?style=for-the-badge&logo=claude&logoColor=white">
</div>


